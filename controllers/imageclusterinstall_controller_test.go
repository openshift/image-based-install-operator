...
var _ = Describe("Reconcile", func() {
	...
	It("succeeds with dual-stack using MachineNetworks with same MAC", func() {
		bmh := bmhInState(bmh_v1alpha1.StateAvailable)
		// Two NICs with same MAC but different IPs (IPv4 and IPv6)
		...
		Expect(infoOut.MachineNetwork).To(HaveLen(2))
		Expect(infoOut.MachineNetwork[0].CIDR.String()).To(Equal("192.168.1.0/24"))
		Expect(infoOut.MachineNetwork[1].CIDR.String()).To(Equal("2001:db8::/64"))
	})
	...
	It("succeeds with dual-stack when BMH has multiple physical NICs with same MAC having correct IPs", func() {
		bmh := bmhInState(bmh_v1alpha1.StateAvailable)
		// Multiple entries with same MAC, one has IPv4, another has IPv6
		bmh.Status.HardwareDetails.NIC = []bmh_v1alpha1.NIC{
			{IP: "10.0.0.30", MAC: "aa:bb:cc:dd:ee:ff"},    // Wrong IPv4 network
			{IP: "192.168.1.30", MAC: "aa:bb:cc:dd:ee:ff"}, // Correct IPv4 network
			{IP: "2001:db8::30", MAC: "aa:bb:cc:dd:ee:ff"}, // Correct IPv6 network
			{IP: "fe80::1", MAC: "aa:bb:cc:dd:ee:ff"},      // Link-local IPv6 (wrong network)
		}
		Expect(c.Create(ctx, bmh)).To(Succeed())
	...
	It("MachineNetworks field takes precedence over legacy MachineNetwork", func() {
		bmh := bmhInState(bmh_v1alpha1.StateAvailable)
		bmh.Status.HardwareDetails.NIC = []bmh_v1alpha1.NIC{
			{IP: "192.168.1.30", MAC: "aa:bb:cc:dd:ee:ff"},
			{IP: "2001:db8::30", MAC: "aa:bb:cc:dd:ee:ff"},
		...
		// Set both legacy and new fields - new should take precedence
		...
		// Should use the new MachineNetworks field, not the legacy one
		Expect(infoOut.MachineNetwork).To(HaveLen(2))
		Expect(infoOut.MachineNetwork[0].CIDR.String()).To(Equal("192.168.1.0/24"))
		Expect(infoOut.MachineNetwork[1].CIDR.String()).To(Equal("2001:db8::/64"))
	})
